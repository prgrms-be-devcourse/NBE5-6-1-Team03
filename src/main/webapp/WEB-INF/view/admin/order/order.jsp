...
<main>
    <div class="section container" id="order-statistics">
        ...
        <h5>${date}</h5>
        <form action="/admin/orders" method="get" style="margin-top: 20px; display: flex; gap: 12px; align-items: center;">
            ...
            <button type="submit" class="btn-small green darken-3 waves-effect waves-light tooltipped"
                    data-position="bottom" data-tooltip="선택한 날짜로 조회합니다" style="height: 45px; line-height: 45px; border-radius: 12px">
                <i class="material-icons left" style="margin-right: 5px;">search</i>조회
            </button>
        </form>

        <c:forEach items="${orderGroupList}" var="group">
            <div class="card white lighten-4 z-depth-1" style="margin-top: 30px; margin-bottom: 30px; border-radius: 12px;">
                ...
                <span style="font-size: 1.4rem; font-weight: 600; color: #2e7d32;">
                        ...
                        <div>
                            <c:if test="${group.items[0].status eq 'ACCEPTED'}">
                                <form action="/admin/orders/status" method="post" style="display: inline;">
                                    ...
                                    <button type="submit" class="btn-floating btn-small blue darken-2 tooltipped"
                                            data-position="left" data-tooltip="그룹 발송" style="margin-right: 10px;">
                    ...
                    <table class="highlight centered" style="margin-top: 20px; font-size: 0.95rem;">
                        ...
                        <c:forEach items="${group.items}" var="item">
                            <tr onclick="location.href='/admin/orders/${item.orderId}?orderDateTime=${item.orderDateTime}'" style="cursor: pointer;">
                                ...
                                <td>
                                    <c:choose>
                                        <c:when test="${item.status eq 'ACCEPTED'}">
                                            <div class="chip blue lighten-4 blue-text text-darken-4">주문접수</div>
                                        </c:when>
                                        <c:when test="${item.status eq 'RELEASE'}">
                                            <div class="chip green lighten-4 green-text text-darken-4">상품발송</div>
                                        </c:when>
                                        <c:when test="${item.status eq 'ERROR'}">
                                            <div class="chip yellow lighten-4 yellow-text text-darken-4">확인필요</div>
                                        </c:when>
                                        <c:otherwise>
